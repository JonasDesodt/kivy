...
class SuggestionlistItem(BoxLayout):
    @inject
    ...
    def delete_track_relationship(self, id):
        track_relationship = self.session.query(TrackRelationship).filter(TrackRelationship.track_relationship_id == id).first()

        if track_relationship :          
            self.session.delete(track_relationship )
            self.session.commit()
